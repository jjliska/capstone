...
def getCenterBox():
  return (facePos[0]+(facePos[2]/2)),(facePos[1]+(facePos[3]/2))

#Bounds for convenience later
boundsRight = cameraRes[0]/3
boundsLeft = cameraRes[0]*2/3
boundsTop = cameraRes[1]/3
boundsBottom = cameraRes[1]*2/3

innerBoundsRight = cameraRes[0]*2/5
innerBoundsLeft = cameraRes[0]*3/5
innerBoundsTop = cameraRes[1]*2/5
innerBoundsBottom = cameraRes[1]*3/5
...
def accelerateMovement(variable,bounded,posNeg):
  ...
  if noFaceDetected >= noFaceTime:
    if bounded:
      if systemVelocity[variable] <= 100.0:
        systemVelocity[variable] = accel*(tn-t0)+systemVelocity[variable]
        if abs(systemVelocity[variable]) > 100.0:
          systemVelocity[variable] = 100.0*posNeg
      else:
        systemVelocity[variable] = 100.0*posNeg
        maxVelReached[variable] = True
    else:
      if systemVelocity[variable] >= maxVelBounded(variable,posNeg):
        maxVelReached[variable] = True
        if maxVelReached[variable]:
          systemVelocity[variable] = maxVelBounded(variable,posNeg)
        else:
          systemVelocity[variable] = accel*(tn-t0)+systemVelocity[variable]
      else:
        systemVelocity[variable] = accel*(tn-t0)+systemVelocity[variable]
  else:
    systemVelocity[variable] = (posNeg*-1)*accel*(tn-t0)+systemVelocity[variable]
  t0 = time.time()
...
def maxVelBounded(variable,posNeg):
  ...
  if variable == 2:
    distCen = abs(x2-cx)
    dist = x1-cx
    maxVel = abs(abs(dist)-distCen)/abs(boundsRight-innerBoundsRight)*100*posNeg
    print(maxVel)
  elif variable == 1:
    distCen = abs(y2-cy)
    dist = y1-cy
